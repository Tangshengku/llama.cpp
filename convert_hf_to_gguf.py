...
@Model.register("BiMamba2ForCausalLM")
class BiMamba2Model(Model):
    model_arch = gguf.MODEL_ARCH.BIMAMBA2
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.endswith(".dt_bias"):
            name = name.rpartition(".dt_bias")[0] + ".dt_proj.bias"
        new_name = self.map_tensor_name(name, (".weight", ".bias", ".wscale", ".wbias"))

        if name.endswith(".A_log"):
            logger.debug("A_log --> A ==> " + new_name)
            data_torch = -torch.exp(data_torch)
        if any(self.match_model_tensor_name(new_name, t, bid, suffix=".weight") for t in [
            gguf.MODEL_TENSOR.SSM_IN,
            gguf.MODEL_TENSOR.SSM_OUT,
        ]):
            print("binarized:", new_name)
            data_torch = self.weight_quant(data_torch)
